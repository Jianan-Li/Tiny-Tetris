...
#define UP 3
#define RIGHT 4
#define BRIGHTNESS_UP 5
#define BRIGHTNESS_DOWN 6
...
ISR(TIMER1_OVF_vect){  
  ...
  interruptCounter1Hz++;
  interruptCounter2Hz++;
  if (interruptCounter1Hz==50) {
    interruptCounter1Hz=0;
    monitorBattery();
  }
...
void setup(){  
  /* a delay here can be used to limit the load when the device is just powered on
     and/or to make it easier to see the serial debugging information */
  delay(1500);
  ...
  // Shut down immediatly if the flag has been set in EEPROM
  checkBattFlag();
}

void loop(){
  /* battery is monitored in the timer interrupt at 1Hz
     if the battery is low for the entire duration of LOW_BATT_COUNTER_LIMIT
     the lowBattShutdown flag is set in the interrupt, and then handled in the main loop*/
  checkLowBatt();
